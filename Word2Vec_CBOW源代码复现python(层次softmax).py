...
class Word2Vec(object):
    def __init__(self):
        # 用于训练的数据集
        self.train_file_name = '/text8'
        # 训练好的词向量
        self.output_file_name = '/text8_vector_test_2.npz'
        # 将处理好的单词直接读取，省去了进行数据集预处理的步骤
        self.read_vocab_file_name = '/text8_vocab_test_1000_5.txt'
        # 将数据集中处理好的单词单独存成文件
    ...
    def Main(self):
        ...
        # 如果没有处理好的文件，那么就先处理，获取了词汇的数据，然后再进行下面的步骤
            ...
            if self.output_file_name.strip() == '':
                ...
                return
            # 初始化网络，特别是创建Haffman树和一些Haffman树需要使用的变量
        self.InitNet()
    ...
    def InitSynThread(self):
        ...
        self.syn0 = np.expand_dims(
            np.random.uniform(-rand_max, rand_max, size=[self.vocab_size * self.layer1_size]), axis=1)
            ...
            while True:
                if word_count - last_word_count > 10000:
                    ...
                    self.alpha = start_alpha * (
                                1 - self.word_count_actual / (self.iter_size * self.train_word_size + 1))
